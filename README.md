...
This project evaluates empirical mode decomposition (EMD) for recording-level term-versus-preterm classification from electrohysterography (EHG). It uses the public Term–Preterm EHG Dataset with Tocogram (TPEHGT), recording-level stratified cross-validation, and interpretable handcrafted features.
...
The **Annotated intervals** strategy comprises both dataset-provided contraction (BC/EC) and dummy (non-contraction; BD/ED) intervals. Across the 26 included recordings from 18 pregnancies, TPEHGT v1.0.0 provides 100 contraction intervals and 100 dummy intervals, giving 200 annotated intervals in total. The fixed-window strategy retains only complete, consecutive, non-overlapping three-minute segments (249 windows); incomplete trailing portions are discarded.
...
├── reproducibility/         # reproducibility and publication-export instructions
...
@article{tilakarathna_preterm_emd,
  title   = {Empirical mode decomposition and interpretable machine learning for preterm birth classification from electrohysterography},
